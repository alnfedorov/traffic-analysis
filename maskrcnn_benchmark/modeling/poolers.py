...
class AdaptiveFeaturePooling(Pooler):
    def __init__(self, output_size, scales, sampling_ratio):
        ...
        Arguments:
            ...
            sampling_ratio (int): sampling ratio for ROIAlign
        """
        super().__init__(output_size, scales, sampling_ratio)
    ...
    def forward(self, x, boxes):
        ...
        dtype, device = x[0].dtype, x[0].device
        result = torch.zeros(
            (num_rois, num_channels, output_size, output_size, num_levels),  # Roi x channels x H x W x level
            dtype=dtype,
            device=device,
        )
        ...
        result = result.max(dim=-1)[0] # max + indices
        return result
